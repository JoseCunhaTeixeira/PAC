...
st.write("🛈 This tab allows you to visualize seismic records, dispersion images and inversion results.")
...
if st.session_state.VIZ_folder_path is not None:
    ...
    if st.session_state.VIZ_mode == 'Signal':
        if input_dir[-1] != '/':
            input_dir += '/'
        if st.session_state.VIZ_folder_path.split('/')[:-2] == input_dir.split('/')[:-1]:
            st.text('')
            st.text('')
        ...
        if output_dir[-1] != '/':
            output_dir += '/'
        if st.session_state.VIZ_folder_path.split('/')[:-4] == output_dir.split('/')[:-1] and "W" in st.session_state.VIZ_folder_path.split('/')[-2]:
            st.text('')
            st.text('')
            st.success(f"📁 Folder selected: **{st.session_state.VIZ_folder_path}**")
