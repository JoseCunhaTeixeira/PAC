    ...
    def initialize_param_space(self, param_space):
        ...
        while unstable:
            ...
            velocity_model = np.column_stack(
                (np.append(thick_vals, 1000), vp_vals, vs_vals, rho_vals)
            )
...
thickness_mins = np.array(
    inversion_params["inversion"]["thickness_mins"], dtype=np.float64
)
thickness_maxs = np.array(
    inversion_params["inversion"]["thickness_maxs"], dtype=np.float64
)
thickness_perturb_stds = np.array(
    inversion_params["inversion"]["thickness_perturb_stds"]
)
...
for mode in modes:
    try:
        data_obs = np.loadtxt(
            f"{folder_path}/xmid{x_mid}/pick/xmid{x_mid}_obs_M{mode}.pvc"
        )
    except:
        continue
...
for mode, fs_obs in zip(modes, fs_obs_per_mode):
    fwd_functions.append(
        lambda state, mode=mode, fs=fs_obs: fwd_function(state, mode, fs)
    )
...
for vs_vals, thick_vals in zip(all_sampled_vs.T, all_sampled_thick.T):
    gm = np.column_stack(
        (thick_vals, vs_vals * VP_VS, vs_vals, 0.32 * vs_vals * VP_VS + 0.77 * 1000)
    )
...
for i, (thick, vp, vs, rho) in enumerate(
    zip(
        best_layered_gm[:, 0],
        best_layered_gm[:, 1],
        best_layered_gm[:, 2],
...
smooth_best_layered_gm = np.column_stack(
    (np.full_like(vp_smooth, dz), vp_smooth, vs_smooth, rho_smooth)
)
...
for i, (thick, vp, vs, rho, std_vp, std_vs, std_rho) in enumerate(
    zip(
        median_layered_gm[:, 0],
        median_layered_gm[:, 1],
        median_layered_gm[:, 2],
        median_layered_gm[:, 3],
        median_layered_std[:, 1],
        median_layered_std[:, 2],
        median_layered_std[:, 3],
    )
):
...
smooth_median_layered_gm = np.column_stack(
    (np.full_like(vp_smooth, dz), vp_smooth, vs_smooth, rho_smooth)
)
smooth_median_layered_std = np.column_stack(
    (np.full_like(vp_smooth, dz), std_vp_smooth, std_vs_smooth, std_rho_smooth)
)
...
for mode, fs_obs in zip(modes, fs_obs_per_mode):
    best_layered_dc = forward_model_disba(
        best_layered_gm[:, 0], best_layered_gm[:, 2], mode, fs_obs
    )
...
for mode, fs_obs in zip(modes, fs_obs_per_mode):
    smooth_best_layered_dc = forward_model_disba(
        smooth_best_layered_gm[:, 0], smooth_best_layered_gm[:, 2], mode, fs_obs
    )
...
for mode, fs_obs in zip(modes, fs_obs_per_mode):
    median_layered_dc = forward_model_disba(
        median_layered_gm[:, 0], median_layered_gm[:, 2], mode, fs_obs
    )
...
for mode, fs_obs in zip(modes, fs_obs_per_mode):
    smooth_median_layered_dc = forward_model_disba(
        smooth_median_layered_gm[:, 0], smooth_median_layered_gm[:, 2], mode, fs_obs
    )
...
for mode, fs_obs in zip(modes, fs_obs_per_mode):
    median_ensemble_dc = forward_model_disba(
        median_ensemble_gm[:, 0], median_ensemble_gm[:, 2], mode, fs_obs
    )
...
FV = np.loadtxt(
    f"{folder_path}/xmid{x_mid}/comp/xmid{x_mid}_dispersion.csv", delimiter=","
)
...
while inRange:
    ...
    if data.period.shape[0] == 0:
        inRange = False
    mode += 1
    full_pred_modes.append(
        np.column_stack((1 / data.period[::-1], data.velocity[::-1] * 1000))
    )
...
display_dispersion_img(
    ...
    full_pred_modes=full_pred_modes,
    path=name_path,
...
for i, (fs_obs, vr_obs, err_obs, mode) in enumerate(
    zip(fs_obs_per_mode, vr_obs_per_mode, err_obs_per_mode, modes)
):
    # Plot inferred data
    d_pred = np.array(results[f"rayleigh_M{mode}.dpred"])
    ...
    ax.plot(
        fs_obs,
        forward_model_disba(best_layered_gm[:, 0], best_layered_gm[:, 2], mode, fs_obs),
        "o",
        color="tab:green",
        label=label,
        markersize=1.5,
...
for gm, misfit in zip(all_sampled_gm, misfits):
    ...
    vs_vals = np.append(vs_vals, vs_vals[-1])
    ax.step(
        vs_vals, depth_vals, color=cmap(norm(misfit)), label="_nolegend_", linewidth=0.5
    )
...
vs_vals = np.append(vs_vals, vs_vals[-1])
ax.step(
    vs_vals, depth_vals, color="green", label="Smooth best layered model", linewidth=1
)
...
ax.step(
    ...
    depth_vals,
    color="tab:orange",
    label="Smooth median layered model",
    linewidth=1,
)
...
vs_vals = np.append(vs_vals, vs_vals[-1])
ax.step(
    vs_vals, depth_vals, color="tab:red", label="Median ensemble model", linewidth=1
)
...
samples = {
    f"$v_{{s{i + 1}}}$ [m/s]": np.array(results[f"space.vs{i + 1}"]).T
    for i in range(nb_layers)
}
samples.update(
    {
        f"$H_{{{i + 1}}}$ [m]": np.array(results[f"space.thick{i + 1}"]).T
        for i in range(nb_layers - 1)
    }
)
...
sys.stdout = sys.__stdout__
print(
    f"\033[92mID {ID} | x_mid {x_mid} | Inversion completed in {toc - tic:.1f} s\033[0m"
)
sys.stdout = log_file
