...
def plot_pseudo_section(FX, fs, xs, wavelength=False):
    ...
    if not wavelength:
        fig = px.imshow(
            np.flipud(FX.T),
            labels=dict(
                x="Position [m]", y="Frequency [Hz]", color="Phase velocity [m/s]"
            ),
            x=xs,
            y=fs,
            aspect="auto",
        ...
        fig = px.imshow(
            np.flipud(FX.T),
            labels=dict(
                x="Position [m]", y="Wavelength [m]", color="Phase velocity [m/s]"
            ),
            x=xs,
            y=fs,
            aspect="auto",
...
def plot_geophones(selected_geophone_positions, geophone_positions, source_position):
    non_selected_geophones = [
        x for x in geophone_positions if x not in selected_geophone_positions
    ]
    ...
    fig.update_layout(
        ...
        xaxis_range=[
            min(source_position, min(geophone_positions)) - 1,
            max(source_position, max(geophone_positions)) + 1,
        ],
        height=200,
    )
...
def display_spectrum_img_fromArray(
    ...
    plt.figure(figsize=(18 * CM, 8 * CM), dpi=_DPI)
    plt.plot(
        fs, abs(SP[:, 0]), fs, abs(SP[:, Nx // 2]), fs, abs(SP[:, -1]), linewidth=0.5
    )
...
def display_dispersion_img(
    ...
    if len(full_pred_modes) > 0:
        for mode in full_pred_modes:
            ...
            fs_mode = mode[:, 0]
            vs_mode = mode[:, 1]
            plt.plot(fs_mode, vs_mode, color="orange", linewidth=1.0)
...
### -----------------------------------------------------------------------------------------------
def display_pseudo_sections(
    obs_section, pred_section, fs, positions, path, zmin=None, zmax=None
):
