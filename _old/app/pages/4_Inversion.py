...
def update_selection():
    if st.session_state.INV_all_selected:
        for status, position in zip(
            st.session_state.INV_status, st.session_state.INV_positions
        ):
            if status == False:
...
def update_all_checkbox():
    if all(
        [
            st.session_state[f"INV_{position}_selected"]
            for position, status in zip(
                st.session_state.INV_positions, st.session_state.INV_status
            )
            if status == False
        ]
    ):
    ...
    if st.session_state.INV_nb_layers is not None:
        for layer in range(1, st.session_state.INV_nb_layers + 1):
            if st.session_state[f"INV_thickness_min_{layer}"] is not None:
                st.session_state[f"INV_thickness_min_{layer}"] = round(
                    st.session_state[f"INV_thickness_min_{layer}"], 2
                )
            ...
            if st.session_state[f"INV_thickness_max_{layer}"] is not None:
                st.session_state[f"INV_thickness_max_{layer}"] = round(
                    st.session_state[f"INV_thickness_max_{layer}"], 2
                )
                if st.session_state[f"INV_thickness_max_{layer}"] <= 0:
                    st.session_state[f"INV_thickness_max_{layer}"] = 0.01
            if (
                st.session_state[f"INV_thickness_min_{layer}"] is not None
                and st.session_state[f"INV_thickness_max_{layer}"] is not None
            ):
                if (
                    st.session_state[f"INV_thickness_max_{layer}"]
                    <= st.session_state[f"INV_thickness_min_{layer}"]
                ):
                    st.session_state[f"INV_thickness_max_{layer}"] = (
                        st.session_state[f"INV_thickness_min_{layer}"] + 0.01
                    )
            if st.session_state[f"INV_thickness_perturb_std_{layer}"] is not None:
                st.session_state[f"INV_thickness_perturb_std_{layer}"] = round(
                    st.session_state[f"INV_thickness_perturb_std_{layer}"], 2
                )
    ...
    if st.session_state.INV_nb_layers is not None:
        for layer in range(1, st.session_state.INV_nb_layers + 1):
            if st.session_state[f"INV_vs_min_{layer}"] is not None:
                st.session_state[f"INV_vs_min_{layer}"] = round(
                    st.session_state[f"INV_vs_min_{layer}"], 2
                )
            ...
            if st.session_state[f"INV_vs_max_{layer}"] is not None:
                st.session_state[f"INV_vs_max_{layer}"] = round(
                    st.session_state[f"INV_vs_max_{layer}"], 2
                )
                if st.session_state[f"INV_vs_max_{layer}"] <= 0:
                    st.session_state[f"INV_vs_max_{layer}"] = 0.01
            if (
                st.session_state[f"INV_vs_min_{layer}"] is not None
                and st.session_state[f"INV_vs_max_{layer}"] is not None
            ):
                if (
                    st.session_state[f"INV_vs_max_{layer}"]
                    <= st.session_state[f"INV_vs_min_{layer}"]
                ):
                    st.session_state[f"INV_vs_max_{layer}"] = (
                        st.session_state[f"INV_vs_min_{layer}"] + 0.01
                    )
            if st.session_state[f"INV_vs_perturb_std_{layer}"] is not None:
                st.session_state[f"INV_vs_perturb_std_{layer}"] = round(
                    st.session_state[f"INV_vs_perturb_std_{layer}"], 2
                )
...
def set_mode():
    if (
        st.session_state.INV_positions is not None
        and st.session_state.INV_status is not None
    ):
        for position, status in zip(
            st.session_state.INV_positions, st.session_state.INV_status
        ):
...
def set_running_params():
    ...
    if st.session_state.INV_n_burnin_iterations is not None:
        st.session_state.INV_n_burnin_iterations = round(
            st.session_state.INV_n_burnin_iterations, 0
        )
...
def handle_select_folder():
    ...
    if st.session_state.INV_selected_folder is not None:
        st.session_state.INV_folder_path = (
            f"{output_dir}/{st.session_state.INV_selected_folder}/"
        )

        # xmids folders
        INV_folders = [
            folder
            for folder in os.listdir(st.session_state.INV_folder_path)
            if os.path.isdir(os.path.join(st.session_state.INV_folder_path, folder))
        ]
...
if st.session_state.INV_folders is None:
    if st.session_state.INV_modes is not None:
        st.session_state.INV_modes = []
    st.info(
        "👆 Please select a folder containing the picked dispersion curves to be inverted."
    )
    st.stop()
    ...
    if os.path.exists(f"{st.session_state.INV_folder_path}/{folder}/pick/"):
        picked_modes = [
            ...
            for fname in os.listdir(
                f"{st.session_state.INV_folder_path}/{folder}/pick/"
            )
...
for mode, count in distinct_modes.items():
    with st.container(border=True):
        ...
        my_bar = st.progress(
            count / len(st.session_state.INV_positions),
            text=f"{count}/{len(st.session_state.INV_positions)} picked",
        )
        if on:
            ws_per_position = []
            vs_per_position = []
            for j, folder in enumerate(st.session_state.INV_folders):
                try:
                    pvc = np.loadtxt(
                        f"{st.session_state.INV_folder_path}/{folder}/pick/{folder}_obs_M{mode}.pvc"
                    )
                    if len(pvc.shape) == 1:
                        pvc = pvc.reshape(1, -1)
                    pvc = np.round(pvc, 2)
            ...
            obs_v_wx = np.full(
                (len(st.session_state.INV_positions), len(obs_ws)), np.nan
            )
            ...
            fig = plot_pseudo_section(
                obs_v_wx, obs_ws, st.session_state.INV_positions, wavelength=True
            )
            st.plotly_chart(fig)
        else:
            fs_per_position = []
            ...
            obs_v_fx = np.full(
                (len(st.session_state.INV_positions), len(obs_fs)), np.nan
            )
...
options = [
    f"M{mode}" for mode in distinct_modes.keys()
]  # To be added in futur versions
st.multiselect("Modes to invert", options, key="INV_modes", on_change=set_mode)
st.markdown(
    "🛈 *The maximum number of selected modes will be inverted if they were picked.*"
)
...
for i, position in enumerate(st.session_state.INV_positions):
    with columns[c]:
        st.session_state.selections.append(
            st.checkbox(
                f"{position}",
                key=f"INV_{position}_selected",
                on_change=update_all_checkbox,
                disabled=st.session_state.INV_status[i],
            )
        )
    c += 1
    if c == num_columns:
...
st.session_state.INV_selection = [
    position
    for position, selection in zip(
        st.session_state.INV_positions, st.session_state.selections
    )
    if selection
]
...
for i in range(1, st.session_state.INV_nb_layers + 1):
    ...
    with st.container(border=True):
        if i == st.session_state.INV_nb_layers:
            st.markdown("**Half-space:**")
            columns = st.columns(3)
            with columns[0]:
                thickness_min = st.number_input(
                    "Min thickness [m]",
                    key=f"INV_thickness_min_{i}",
                    value=None,
                    disabled=True,
                    placeholder="∞",
                )
            ...
            with columns[2]:
                perturb_std = st.number_input(
                    "Perturbation std [m]",
                    key=f"INV_thickness_perturb_std_{i}",
                    value=None,
                    disabled=True,
                    placeholder="0",
                )
        ...
        columns = st.columns(3)
        with columns[0]:
            vs_min = st.number_input(
                "Min shear wave velocity [m/s]",
                key=f"INV_vs_min_{i}",
                value=None,
                step=1.0,
                placeholder="Enter a value",
        ...
        with columns[1]:
            vs_max = st.number_input(
                "Max shear wave velocity [m/s]",
                key=f"INV_vs_max_{i}",
                value=None,
                step=1.0,
                placeholder="Enter a value",
        ...
        with columns[2]:
            perturb_std = st.number_input(
                "Perturbation std [m/s]",
                key=f"INV_vs_perturb_std_{i}",
                value=None,
                step=1.0,
                placeholder="Enter a value",
                format="%.2f",
...
if (
    None in thickness_mins[:-1]
    or None in thickness_maxs[:-1]
    or None in vs_mins
    or None in vs_maxs
    or None in thickness_perturb_stds[:-1]
    or None in vs_perturb_stds
):
    st.text("")
    st.text("")
    if st.session_state.INV_n_iterations is not None:
...
st.number_input(
    "Iterations [#]",
    key="INV_n_iterations",
    value=None,
    step=1,
    on_change=set_running_params,
    placeholder="Enter a value",
    format="%i",
)
st.number_input(
    "Burning iterations [#]",
    key="INV_n_burnin_iterations",
    value=None,
    step=1,
    on_change=set_running_params,
    placeholder="Enter a value",
...
if (
    st.session_state.INV_n_iterations is None
    or st.session_state.INV_n_burnin_iterations is None
    or st.session_state.INV_n_chains is None
):
    st.text("")
    st.text("")
...
if st.button("Compute", type="primary", use_container_width=True):
    ...
    loading_message.warning("⏳⚙️ Running inversion...")
    ...
    params = {
        ...
        "inversion": {
            "modes": sorted(
                [int(mode.split()[0][1:]) for mode in st.session_state.INV_modes]
            ),
    ...
    params["inversion"]["n_burnin_iterations"] = (
        st.session_state.INV_n_burnin_iterations
    )
    ...
    scripts = [
        f"{work_dir}/scripts/run_inversion.py -ID {i} -r {st.session_state.INV_folder_path}"
        for i in range(st.session_state.INV_nb_scripts)
    ]
    ...
    for folder in st.session_state.INV_folders:
        try:
            gm = np.loadtxt(
                f"{st.session_state.INV_folder_path}/{folder}/inv/{folder}_smooth_median_layered_model.gm",
                skiprows=1,
            )
            std = np.loadtxt(
                f"{st.session_state.INV_folder_path}/{folder}/inv/{folder}_smooth_median_layered_std.gm",
                skiprows=1,
            )
        except:
            gm = None
    ...
    if not all(gm is None for gm in all_gm) and not all(gm is None for gm in all_std):
        ...
        if not all(gm is None for gm in all_gm) and not all(
            gm is None for gm in all_std
        ):
            ...
            std_xd_smooth = generic_filter(
                std_xd_smooth, mode_filter_median, size=(3, 1)
            )
            std_xd_smooth = generic_filter(
                std_xd_smooth, mode_filter_median, size=(2, 1)
            )
            path_name = f"{st.session_state.INV_folder_path}/vs_section.svg"
            display_inverted_section(
                v_xd_smooth,
                std_xd_smooth,
                st.session_state.INV_positions,
                depths,
                path_name,
            )
    ...
    for mode in st.session_state.INV_modes:
        ...
        for j, folder in enumerate(st.session_state.INV_folders):
            try:
                pvc = np.loadtxt(
                    f"{st.session_state.INV_folder_path}/{folder}/pick/{folder}_obs_{mode}.pvc"
                )
                if len(pvc.shape) == 1:
                    pvc = pvc.reshape(1, -1)
                pvc = np.round(pvc, 2)
        ...
        for j, folder in enumerate(st.session_state.INV_folders):
            try:
                pvc = np.loadtxt(
                    f"{st.session_state.INV_folder_path}/{folder}/inv/{folder}_smooth_median_layered_{mode}.pvc"
                )
                if len(pvc.shape) == 1:
                    pvc = pvc.reshape(1, -1)
                pvc = np.round(pvc, 2)
        ...
        if len(obs_fs_per_position) > 0 and len(pred_fs_per_position) > 0:
            ...
            path_name = (
                f"{st.session_state.INV_folder_path}/{mode}_vr_pseudo-section.svg"
            )
            display_pseudo_sections(
                obs_v_fx, pred_v_fx, fs, st.session_state.INV_positions, path_name
            )
    ...
    if nb_sucess == st.session_state.INV_nb_scripts:
        st.success(
            f"👌 Inversion completed for all {st.session_state.INV_nb_scripts} MASW postions."
        )
    elif nb_sucess == 0:
        st.error(
            f"❌ Inversion failed for all {st.session_state.INV_nb_scripts} MASW postions."
        )
    else:
        st.warning(
            f"⚠️ Inversion only completed for {nb_sucess} over {st.session_state.INV_nb_scripts} MASW postions."
        )
