...
from dispersion import resamp_frequency, resamp_wavelength
from display import (
    display_inverted_section,
    display_pseudo_sections,
    plot_disp,
    plot_inverted_section,
    plot_pseudo_section,
    plot_std_section,
    plot_wiggle,
)
...
def load_sections():
    ...
    for folder in st.session_state.VIZ_folders:
        try:
            gm = np.loadtxt(
                f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_median_layered_model.gm",
                skiprows=1,
            )
            std = np.loadtxt(
                f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_median_layered_std.gm",
                skiprows=1,
            )
        except:
            gm = None
    ...
    for folder in st.session_state.VIZ_folders:
        try:
            gm = np.loadtxt(
                f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_smooth_median_layered_model.gm",
                skiprows=1,
            )
            std = np.loadtxt(
                f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_smooth_median_layered_std.gm",
                skiprows=1,
            )
        except:
            gm = None
    ...
    for folder in st.session_state.VIZ_folders:
        try:
            gm = np.loadtxt(
                f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_best_layered_model.gm",
                skiprows=1,
            )
        except:
            gm = None
    ...
    for folder in st.session_state.VIZ_folders:
        try:
            gm = np.loadtxt(
                f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_smooth_best_layered_model.gm",
                skiprows=1,
            )
        except:
            gm = None
    ...
    for folder in st.session_state.VIZ_folders:
        try:
            gm = np.loadtxt(
                f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_median_ensemble_model.gm",
                skiprows=1,
            )
            std = np.loadtxt(
                f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_median_ensemble_std.gm",
                skiprows=1,
            )
        except:
            gm = None
    ...
    if not all(gm is None for gm in all_gm_median_layered):
        max_depth_median_layered = np.nanmax(
            [np.sum(gm[:, 0]) for gm in all_gm_median_layered if gm is not None]
        )
    else:
        max_depth_median_layered = np.nan
    if not all(gm is None for gm in all_gm_verticalsmooth_median_layered):
        max_depth_vertical_smooth_median_layered = np.nanmax(
            [
                np.sum(gm[:, 0])
                for gm in all_gm_verticalsmooth_median_layered
                if gm is not None
            ]
        )
    else:
    ...
    if not all(gm is None for gm in all_gm_best_layered):
        max_depth_best_layered = np.nanmax(
            [np.sum(gm[:, 0]) for gm in all_gm_best_layered if gm is not None]
        )
    else:
        max_depth_best_layered = np.nan
    if not all(gm is None for gm in all_gm_verticalsmooth_best_layered):
        max_depth_vertical_smooth_best_layered = np.nanmax(
            [
                np.sum(gm[:, 0])
                for gm in all_gm_verticalsmooth_best_layered
                if gm is not None
            ]
        )
    else:
    ...
    if not all(gm is None for gm in all_gm_median_ensemble):
        max_depth_median_ensemble = np.nanmax(
            [np.sum(gm[:, 0]) for gm in all_gm_median_ensemble if gm is not None]
        )
    else:
        max_depth_median_ensemble = np.nan
    depth_max = np.nanmax(
        [
            max_depth_median_layered,
            max_depth_vertical_smooth_median_layered,
            max_depth_best_layered,
            max_depth_vertical_smooth_best_layered,
            max_depth_median_ensemble,
        ]
    )

    if not all(gm is None for gm in all_gm_median_layered) and not all(
        gm is None for gm in all_std_median_layered
    ):
        ...
        st.session_state.VIZ_v_xd_median_layered = np.full(
            (
                len(st.session_state.VIZ_xmids),
                len(st.session_state.VIZ_depths_median_layered),
            ),
            np.nan,
        )
        st.session_state.VIZ_std_xd_median_layered = np.full(
            (
                len(st.session_state.VIZ_xmids),
                len(st.session_state.VIZ_depths_median_layered),
            ),
        ...
        for j, (gm, gm_std) in enumerate(
            zip(all_gm_median_layered, all_std_median_layered)
        ):
            if gm is not None and gm_std is not None:
                col = []
                col_std = []
                for (thick, vp, vs, rho), (thick_std, vp_std, vs_std, rho_std) in zip(
                    gm, gm_std
                ):
                    col += [vs] * int(thick / dz)
                ...
                if len(col) < len(st.session_state.VIZ_depths_median_layered):
                    col += [gm[-1, 2]] * (
                        len(st.session_state.VIZ_depths_median_layered) - len(col)
                    )
                    col_std += [gm_std[-1, 2]] * (
                        len(st.session_state.VIZ_depths_median_layered) - len(col_std)
                    )
                st.session_state.VIZ_v_xd_median_layered[j, :] = col
                st.session_state.VIZ_std_xd_median_layered[j, :] = col_std

        st.session_state.VIZ_vs_min_median_layered = np.floor(
            np.nanmin(st.session_state.VIZ_v_xd_median_layered)
        )
        st.session_state.VIZ_vs_max_median_layered = np.ceil(
            np.nanmax(st.session_state.VIZ_v_xd_median_layered)
        )

        st.session_state.VIZ_v_xd_median_layered_lateralsmooth = generic_filter(
            st.session_state.VIZ_v_xd_median_layered, mode_filter_median, size=(4, 1)
        )
        st.session_state.VIZ_v_xd_median_layered_lateralsmooth = generic_filter(
            st.session_state.VIZ_v_xd_median_layered_lateralsmooth,
            mode_filter_median,
            size=(3, 1),
        )
        st.session_state.VIZ_v_xd_median_layered_lateralsmooth = generic_filter(
        ...
        st.session_state.VIZ_std_xd_median_layered_lateralsmooth = generic_filter(
            st.session_state.VIZ_std_xd_median_layered, mode_filter_median, size=(4, 1)
        )
        st.session_state.VIZ_std_xd_median_layered_lateralsmooth = generic_filter(
            st.session_state.VIZ_std_xd_median_layered_lateralsmooth,
            mode_filter_median,
            size=(3, 1),
        )
        st.session_state.VIZ_std_xd_median_layered_lateralsmooth = generic_filter(
    ...
    if not all(gm is None for gm in all_gm_verticalsmooth_median_layered) and not all(
        gm is None for gm in all_std_verticalsmooth_median_layered
    ):
        if all_gm_verticalsmooth_median_layered[0][0, 0] < 0.1:
            dz = all_gm_verticalsmooth_median_layered[0][0, 0]
        else:
            dz = 0.1
        st.session_state.VIZ_depths_verticalsmooth_median_layered = arange(
            0, depth_max, dz
        )

        st.session_state.VIZ_v_xd_verticalsmooth_median_layered = np.full(
            (
                len(st.session_state.VIZ_xmids),
                len(st.session_state.VIZ_depths_verticalsmooth_median_layered),
            ),
            np.nan,
        )
        st.session_state.VIZ_std_xd_verticalsmooth_median_layered = np.full(
            (
                len(st.session_state.VIZ_xmids),
                len(st.session_state.VIZ_depths_verticalsmooth_median_layered),
            ),
        ...
        for j, (gm, gm_std) in enumerate(
            ...
            if gm is not None and gm_std is not None:
                ...
                for (thick, vp, vs, rho), (thick_std, vp_std, vs_std, rho_std) in zip(
                    ...
                    col_std += [vs_std] * int(thick / dz)
                if len(col) < len(
                    st.session_state.VIZ_depths_verticalsmooth_median_layered
                ):
                    col += [gm[-1, 2]] * (
                        len(st.session_state.VIZ_depths_verticalsmooth_median_layered)
                        - len(col)
                ...
                st.session_state.VIZ_std_xd_verticalsmooth_median_layered[j, :] = (
                    col_std
                )

        st.session_state.VIZ_vs_min_verticalsmooth_median_layered = np.floor(
            np.nanmin(st.session_state.VIZ_v_xd_verticalsmooth_median_layered)
        )
        st.session_state.VIZ_vs_max_verticalsmooth_median_layered = np.ceil(
            np.nanmax(st.session_state.VIZ_v_xd_verticalsmooth_median_layered)
        )

        st.session_state.VIZ_v_xd_verticalsmooth_median_layered_lateralsmooth = (
            generic_filter(
                st.session_state.VIZ_v_xd_verticalsmooth_median_layered,
                mode_filter_median,
                size=(4, 1),
            )
        )
        ...
        st.session_state.VIZ_std_xd_verticalsmooth_median_layered_lateralsmooth = (
            generic_filter(
                st.session_state.VIZ_std_xd_verticalsmooth_median_layered,
                mode_filter_median,
                size=(4, 1),
            )
        )
    ...
    if not all(gm is None for gm in all_gm_best_layered):
        ...
        st.session_state.VIZ_v_xd_best_layered = np.full(
            (
                len(st.session_state.VIZ_xmids),
                len(st.session_state.VIZ_depths_best_layered),
            ),
            np.nan,
        )
        ...
        for j, gm in enumerate(all_gm_best_layered):
            if gm is not None:
                ...
                if len(col) < len(st.session_state.VIZ_depths_best_layered):
                    col += [gm[-1, 2]] * (
                        len(st.session_state.VIZ_depths_best_layered) - len(col)
                    )
                st.session_state.VIZ_v_xd_best_layered[j, :] = col

        st.session_state.VIZ_vs_min_best_layered = np.floor(
            np.nanmin(st.session_state.VIZ_v_xd_best_layered)
        )
        st.session_state.VIZ_vs_max_best_layered = np.ceil(
            np.nanmax(st.session_state.VIZ_v_xd_best_layered)
        )

        st.session_state.VIZ_v_xd_best_layered_lateralsmooth = generic_filter(
            st.session_state.VIZ_v_xd_best_layered, mode_filter_median, size=(4, 1)
        )
        st.session_state.VIZ_v_xd_best_layered_lateralsmooth = generic_filter(
            st.session_state.VIZ_v_xd_best_layered_lateralsmooth,
            mode_filter_median,
            size=(3, 1),
        )
        st.session_state.VIZ_v_xd_best_layered_lateralsmooth = generic_filter(
    ...
    if not all(gm is None for gm in all_gm_verticalsmooth_best_layered):
        if all_gm_verticalsmooth_best_layered[0][0, 0] < 0.1:
            ...
            dz = 0.1
        st.session_state.VIZ_depths_verticalsmooth_best_layered = arange(
            0, depth_max, dz
        )

        st.session_state.VIZ_v_xd_verticalsmooth_best_layered = np.full(
            (
                len(st.session_state.VIZ_xmids),
                len(st.session_state.VIZ_depths_verticalsmooth_best_layered),
            ),
            np.nan,
        )
        ...
        for j, gm in enumerate(all_gm_verticalsmooth_best_layered):
            if gm is not None:
                ...
                for thick, vp, vs, rho in gm:
                    col += [vs] * int(thick / dz)
                if len(col) < len(
                    st.session_state.VIZ_depths_verticalsmooth_best_layered
                ):
                    col += [gm[-1, 2]] * (
                        len(st.session_state.VIZ_depths_verticalsmooth_best_layered)
                        - len(col)
        ...
        st.session_state.VIZ_vs_min_verticalsmooth_best_layered = np.floor(
            np.nanmin(st.session_state.VIZ_v_xd_verticalsmooth_best_layered)
        )
        st.session_state.VIZ_vs_max_verticalsmooth_best_layered = np.ceil(
            np.nanmax(st.session_state.VIZ_v_xd_verticalsmooth_best_layered)
        )

        st.session_state.VIZ_v_xd_verticalsmooth_best_layered_lateralsmooth = (
            generic_filter(
                st.session_state.VIZ_v_xd_verticalsmooth_best_layered,
                mode_filter_median,
                size=(4, 1),
            )
        )
    ...
    if not all(gm is None for gm in all_gm_median_ensemble) and not all(
        gm is None for gm in all_std_median_ensemble
    ):
        ...
        st.session_state.VIZ_v_xd_median_ensemble = np.full(
            (
                len(st.session_state.VIZ_xmids),
                len(st.session_state.VIZ_depths_median_ensemble),
            ),
            np.nan,
        )
        st.session_state.VIZ_std_xd_median_ensemble = np.full(
            (
                len(st.session_state.VIZ_xmids),
                len(st.session_state.VIZ_depths_median_ensemble),
            ),
        ...
        for j, (gm, gm_std) in enumerate(
            zip(all_gm_median_ensemble, all_std_median_ensemble)
        ):
            if gm is not None and gm_std is not None:
                col = []
                col_std = []
                ...
                if len(col) < len(st.session_state.VIZ_depths_median_ensemble):
                    col += [gm[-1, 2]] * (
                        len(st.session_state.VIZ_depths_median_ensemble) - len(col)
                    )
                    col_std += [gm_std[-1, 2]] * (
                        len(st.session_state.VIZ_depths_median_ensemble) - len(col_std)
                    )
                st.session_state.VIZ_v_xd_median_ensemble[j, :] = col
                st.session_state.VIZ_std_xd_median_ensemble[j, :] = col_std

        st.session_state.VIZ_vs_min_median_ensemble = np.floor(
            np.nanmin(st.session_state.VIZ_v_xd_median_ensemble)
        )
        st.session_state.VIZ_vs_max_median_ensemble = np.ceil(
            np.nanmax(st.session_state.VIZ_v_xd_median_ensemble)
        )

        st.session_state.VIZ_v_xd_median_ensemble_lateralsmooth = generic_filter(
            st.session_state.VIZ_v_xd_median_ensemble, mode_filter_median, size=(4, 1)
        )
        st.session_state.VIZ_v_xd_median_ensemble_lateralsmooth = generic_filter(
            st.session_state.VIZ_v_xd_median_ensemble_lateralsmooth,
            mode_filter_median,
            size=(3, 1),
        )
        st.session_state.VIZ_v_xd_median_ensemble_lateralsmooth = generic_filter(
        ...
        st.session_state.VIZ_std_xd_median_ensemble_lateralsmooth = generic_filter(
            st.session_state.VIZ_std_xd_median_ensemble, mode_filter_median, size=(4, 1)
        )
        st.session_state.VIZ_std_xd_median_ensemble_lateralsmooth = generic_filter(
            st.session_state.VIZ_std_xd_median_ensemble_lateralsmooth,
            mode_filter_median,
            size=(3, 1),
        )
        st.session_state.VIZ_std_xd_median_ensemble_lateralsmooth = generic_filter(
...
def save_images():
    ...
    for folder, pos in zip(st.session_state.VIZ_folders, st.session_state.VIZ_xmids):
        if os.path.exists(f"{st.session_state.VIZ_folder_path}/{folder}/pick/"):
            picked_modes = [
                int(fname.split("_M")[1].split(".")[0])
                for fname in os.listdir(
                    f"{st.session_state.VIZ_folder_path}/{folder}/pick"
                )
                if fname.endswith(".pvc") and fname.startswith(f"xmid{pos}_obs_M")
            ]
    ...
    if st.session_state.VIZ_model == "Median layered":
        if st.session_state.VIZ_vertical_smoothing == "No":
            if st.session_state.VIZ_lateral_smoothing == "No":
                if st.session_state.VIZ_v_xd_median_layered is not None:
                    display_inverted_section(
                        vs_section=st.session_state.VIZ_v_xd_median_layered,
                ...
                if st.session_state.VIZ_v_xd_median_layered_lateralsmooth is not None:
                    display_inverted_section(
                        vs_section=st.session_state.VIZ_v_xd_median_layered_lateralsmooth,
            ...
            if st.session_state.VIZ_lateral_smoothing == "No":
                if st.session_state.VIZ_v_xd_verticalsmooth_median_layered is not None:
                    display_inverted_section(
                        vs_section=st.session_state.VIZ_v_xd_verticalsmooth_median_layered,
                ...
                if (
                    ...
                    display_inverted_section(
                        vs_section=st.session_state.VIZ_v_xd_verticalsmooth_median_layered_lateralsmooth,
        ...
        for mode in distinct_modes.keys():
            ...
            for j, folder in enumerate(st.session_state.VIZ_folders):
                try:
                    pvc = np.loadtxt(
                        f"{st.session_state.VIZ_folder_path}/{folder}/pick/{folder}_obs_M{mode}.pvc"
                    )
                    if len(pvc.shape) == 1:
                        pvc = pvc.reshape(1, -1)
                    pvc = np.round(pvc, 2)
            ...
            for j, folder in enumerate(st.session_state.VIZ_folders):
                try:
                    if st.session_state.VIZ_vertical_smoothing == "No":
                        pvc = np.loadtxt(
                            f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_median_layered_M{mode}.pvc"
                        )
                    elif st.session_state.VIZ_vertical_smoothing == "Yes":
                        pvc = np.loadtxt(
                            f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_smooth_median_layered_M{mode}.pvc"
                        )
                    if len(pvc.shape) == 1:
                        pvc = pvc.reshape(1, -1)
                    pvc = np.round(pvc, 2)
            ...
            # General
            obs_f_min = np.min(
                [np.min(fs) for fs in obs_fs_per_position if len(fs) > 0]
            )
            obs_f_max = np.max(
                [np.max(fs) for fs in obs_fs_per_position if len(fs) > 0]
            )
            if not all(len(fs) == 0 for fs in pred_fs_per_position):
                pred_f_min = np.min(
                    [np.min(fs) for fs in pred_fs_per_position if len(fs) > 0]
                )
                pred_f_max = np.max(
                    [np.max(fs) for fs in pred_fs_per_position if len(fs) > 0]
                )
            else:
            ...
            for j, (obs_fs, obs_vs, pred_fs, pred_vs) in enumerate(
                zip(
                    obs_fs_per_position,
                    obs_vs_per_position,
                    pred_fs_per_position,
            ...
            path_name = (
                f"{st.session_state.VIZ_folder_path}/M{mode}_vr_pseudo-section.svg"
            )
            display_pseudo_sections(
                obs_v_fx, pred_v_fx, fs, st.session_state.VIZ_xmids, path_name
            )
        ...
        if st.session_state.VIZ_vertical_smoothing == "No":
            if st.session_state.VIZ_lateral_smoothing == "No":
                if st.session_state.VIZ_v_xd_best_layered is not None:
                    display_inverted_section(
                        vs_section=st.session_state.VIZ_v_xd_best_layered,
                        std_section=np.full(
                            st.session_state.VIZ_v_xd_best_layered.shape, np.nan
                        ),
                        positions=st.session_state.VIZ_xmids,
                ...
                if st.session_state.VIZ_v_xd_best_layered_lateralsmooth is not None:
                    display_inverted_section(
                        vs_section=st.session_state.VIZ_v_xd_best_layered_lateralsmooth,
                        std_section=np.full(
                            st.session_state.VIZ_v_xd_best_layered_lateralsmooth.shape,
                            np.nan,
                        ),
                        positions=st.session_state.VIZ_xmids,
                        depths=st.session_state.VIZ_depths_best_layered,
            ...
            if st.session_state.VIZ_lateral_smoothing == "No":
                if st.session_state.VIZ_v_xd_verticalsmooth_best_layered is not None:
                    display_inverted_section(
                        vs_section=st.session_state.VIZ_v_xd_verticalsmooth_best_layered,
                        std_section=np.full(
                            st.session_state.VIZ_v_xd_verticalsmooth_best_layered.shape,
                            np.nan,
                        ),
                        positions=st.session_state.VIZ_xmids,
                ...
                if (
                    ...
                    display_inverted_section(
                        vs_section=st.session_state.VIZ_v_xd_verticalsmooth_best_layered_lateralsmooth,
                        std_section=np.full(
                            st.session_state.VIZ_v_xd_verticalsmooth_best_layered_lateralsmooth.shape,
                            np.nan,
                        ),
                        positions=st.session_state.VIZ_xmids,
                        depths=st.session_state.VIZ_depths_verticalsmooth_best_layered,
        ...
        for mode in distinct_modes.keys():
            ...
            for j, folder in enumerate(st.session_state.VIZ_folders):
                try:
                    if st.session_state.VIZ_vertical_smoothing == "No":
                        pvc = np.loadtxt(
                            f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_best_layered_M{mode}.pvc"
                        )
                    elif st.session_state.VIZ_vertical_smoothing == "Yes":
                        pvc = np.loadtxt(
                            f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_smooth_best_layered_M{mode}.pvc"
                        )
                    if len(pvc.shape) == 1:
                        pvc = pvc.reshape(1, -1)
                    pvc = np.round(pvc, 2)
    ...
    if st.session_state.VIZ_model == "Median ensemble":
        if st.session_state.VIZ_lateral_smoothing == "No":
            if st.session_state.VIZ_v_xd_median_ensemble is not None:
                display_inverted_section(
                    vs_section=st.session_state.VIZ_v_xd_median_ensemble,
            ...
            if st.session_state.VIZ_v_xd_median_ensemble_lateralsmooth is not None:
                display_inverted_section(
                    vs_section=st.session_state.VIZ_v_xd_median_ensemble_lateralsmooth,
        ...
        for mode in distinct_modes.keys():
            ...
            for j, folder in enumerate(st.session_state.VIZ_folders):
                try:
                    pvc = np.loadtxt(
                        f"{st.session_state.VIZ_folder_path}/{folder}/inv/{folder}_median_ensemble_M{mode}.pvc"
                    )
                    if len(pvc.shape) == 1:
                        pvc = pvc.reshape(1, -1)
                    pvc = np.round(pvc, 2)
...
def handle_select_folder():
    ...
    if st.session_state.VIZ_selected_folder is not None:
        if st.session_state.VIZ_mode == "Signal":
            st.session_state.VIZ_folder_path = (
                f"{input_dir}/{st.session_state.VIZ_selected_folder}/"
            )
        if st.session_state.VIZ_mode in ["Dispersion", "Inversion"]:
            st.session_state.VIZ_folder_path = (
                f"{output_dir}/{st.session_state.VIZ_selected_folder}/"
            )
...
st.write(
    "🛈 Visualization of raw seismic records, dispersion images and inversion results."
)
...
# Option
st.selectbox(
    options=["Signal", "Dispersion", "Inversion"],
    key="VIZ_mode",
    label="**Visualization mode**",
    index=None,
    placeholder="Select",
    on_change=handle_select_mode,
)
...
if st.session_state.VIZ_mode == "Signal":
    ...
    if input_folders:
        st.selectbox(
            "**Data folder**",
            input_folders,
            key="VIZ_selected_folder",
            on_change=handle_select_folder,
            index=None,
            placeholder="Select",
        )
    else:
    ...
    files = [
        file
        for file in os.listdir(st.session_state.VIZ_folder_path)
    ...
    st.session_state.VIZ_durations = [
        stream[0].stats.endtime - stream[0].stats.starttime
        for stream in st.session_state.VIZ_streams
    ]
    ...
    for i, (stream, file) in enumerate(zip(st.session_state.VIZ_streams, files)):
        ...
        if total_points > 1000:
            ...
            TX = TX[::factor, :]
        delta = delta * factor
        x_receivers = arange(1, len(stream), 1)
        fig = plot_wiggle(TX.T, x_receivers, delta, norm=norm)
        st.text("")
        st.text("")
        st.text("")
        st.text("")
    ...
    folders = [
        xmid
        for xmid in os.listdir(st.session_state.VIZ_folder_path)
        if xmid[0:4] == "xmid"
    ]
    ...
    st.session_state.VIZ_xmids, st.session_state.VIZ_folders = zip(
        *sorted(zip(xmids, folders))
    )
    ...
    with open(f"{st.session_state.VIZ_folder_path}/computing_params.json", "r") as f:
        ...
        st.session_state.VIZ_dx = (
            computing_param["positions"][1] - computing_param["positions"][0]
        )
    ...
    for folder in st.session_state.VIZ_folders:
        ...
        if (
            f"{folder}_dispersion.csv" in list_dir
            and f"{folder}_fs.csv" in list_dir
            and f"{folder}_vs.csv" in list_dir
        ):
            all_dispersion.append(
                np.loadtxt(
                    f"{st.session_state.VIZ_folder_path}/{folder}/comp/{folder}_dispersion.csv",
                    delimiter=",",
                )
            )
            all_fs.append(
                np.loadtxt(
                    f"{st.session_state.VIZ_folder_path}/{folder}/comp/{folder}_fs.csv",
                    delimiter=",",
                )
            )
            all_vs.append(
                np.loadtxt(
                    f"{st.session_state.VIZ_folder_path}/{folder}/comp/{folder}_vs.csv",
                    delimiter=",",
                )
            )
    ...
    for available, xmid, dispersion, fs, vs in zip(
        availables, st.session_state.VIZ_xmids, all_dispersion, all_fs, all_vs
    ):
        if available:
            fig = plot_disp(
                dispersion,
                ...
                vs,
                norm="Frequencies",
                dx=st.session_state.VIZ_dx,
                Nx=st.session_state.VIZ_Nx,
            )
            ...
            if os.path.exists(f"{st.session_state.VIZ_folder_path}/xmid{xmid}/pick/"):
                picked_modes = [
                    fname
                    for fname in os.listdir(
                        f"{st.session_state.VIZ_folder_path}/xmid{xmid}/pick/"
                    )
                    if fname.endswith(".pvc") and fname.startswith(f"xmid{xmid}_obs_M")
                ]
            else:
                picked_modes = []
            for picked_mode in picked_modes:
                pvc = np.loadtxt(
                    f"{st.session_state.VIZ_folder_path}/xmid{xmid}/pick/{picked_mode}"
                )
                if len(pvc.shape) == 1:
                    pvc = pvc.reshape(1, -1)
                fig.add_trace(
            ...
            st.text("")
            st.markdown(f"**Position: {xmid} m**")
            st.error("📁 Dispersion data missing.")
    ...
    for mode, count in distinct_modes.items():
        if on:
            ...
            fig = plot_pseudo_section(
                obs_v_wx, obs_ws, st.session_state.VIZ_xmids, wavelength=True
            )
            st.plotly_chart(fig)
        else:
            fs_per_position = []
    ...
    if (
        st.session_state.VIZ_v_xd_median_ensemble is None
        and st.session_state.VIZ_v_xd_median_layered is None
        and st.session_state.VIZ_v_xd_best_layered is None
    ):
        load_sections()
    ...
    if (
        ...
        st.text("")
        st.text("")
        st.error("📁 Inversion data missing.")
        st.divider()
        st.stop()
    ...
    with st.container(border=True):
        ...
        with columns[0]:
            st.radio(
                "**Colormap**", ["Terrain", "Viridis", "Jet", "Gray"], key="VIZ_cmap"
            )
        with columns[1]:
            st.radio(
                "**Model**",
                ["Median layered", "Best layered", "Median ensemble"],
                key="VIZ_model",
            )
        with columns[2]:
            st.radio(
                "**Vertical smoothing**", ["Yes", "No"], key="VIZ_vertical_smoothing"
            )
        with columns[3]:
            st.radio(
                "**Lateral smoothing**", ["Yes", "No"], key="VIZ_lateral_smoothing"
            )
        ...
        if st.session_state.VIZ_v_xd_median_ensemble is not None:
            ...
            if st.session_state.VIZ_lateral_smoothing == "Yes":
                fig = plot_inverted_section(
                    st.session_state.VIZ_v_xd_median_ensemble_lateralsmooth,
                    st.session_state.VIZ_xmids,
                    st.session_state.VIZ_depths_median_ensemble,
                    zmin=vmin,
                    zmax=vmax,
                )
            elif st.session_state.VIZ_lateral_smoothing == "No":
                fig = plot_inverted_section(
                    st.session_state.VIZ_v_xd_median_ensemble,
                    st.session_state.VIZ_xmids,
                    st.session_state.VIZ_depths_median_ensemble,
                    zmin=vmin,
                    zmax=vmax,
            ...
            if st.session_state.VIZ_cmap is not None:
                if st.session_state.VIZ_cmap == "Terrain":
                    ...
                    colorscale = [
                        (i / 255.0, mcolors.rgb2hex(cmap(i / 255.0)))
                        for i in range(256)
                    ]
            ...
            if st.session_state.VIZ_lateral_smoothing == "Yes":
                ...
                fig = plot_std_section(
                    st.session_state.VIZ_std_xd_median_ensemble,
                    st.session_state.VIZ_xmids,
                    st.session_state.VIZ_depths_median_ensemble,
                )
            fig.update_layout(height=400)
            ...
            st.markdown(
                r"🛈 *Will save the above $v_{S}$ and standard deviation sections, and the corresponding $v_{R}$ pseudo-sections.*"
            )
            ...
            st.text("")
            st.text("")
            st.error("📁 Inversion data missing for median ensemble model.")
            ...
            if st.session_state.VIZ_v_xd_median_layered is not None:
                ...
                if st.session_state.VIZ_lateral_smoothing == "Yes":
                    fig = plot_inverted_section(
                        st.session_state.VIZ_v_xd_median_layered_lateralsmooth,
                        st.session_state.VIZ_xmids,
                        st.session_state.VIZ_depths_median_layered,
                        zmin=vmin,
                        zmax=vmax,
                    )
                elif st.session_state.VIZ_lateral_smoothing == "No":
                    fig = plot_inverted_section(
                        st.session_state.VIZ_v_xd_median_layered,
                        st.session_state.VIZ_xmids,
                        st.session_state.VIZ_depths_median_layered,
                        zmin=vmin,
                        zmax=vmax,
                ...
                if st.session_state.VIZ_lateral_smoothing == "Yes":
                    ...
                    fig = plot_std_section(
                        st.session_state.VIZ_std_xd_median_layered,
                        st.session_state.VIZ_xmids,
                        st.session_state.VIZ_depths_median_layered,
                    )
                fig.update_layout(height=400)
                ...
                st.text("")
                st.text("")
                st.error("📁 Inversion data missing for median layered model.")
            ...
            if st.session_state.VIZ_v_xd_best_layered is not None:
                ...
                if st.session_state.VIZ_lateral_smoothing == "Yes":
                    fig = plot_inverted_section(
                        st.session_state.VIZ_v_xd_best_layered_lateralsmooth,
                        st.session_state.VIZ_xmids,
                        st.session_state.VIZ_depths_best_layered,
                        zmin=vmin,
                        zmax=vmax,
                    )
                elif st.session_state.VIZ_lateral_smoothing == "No":
                    fig = plot_inverted_section(
                        st.session_state.VIZ_v_xd_best_layered,
                        st.session_state.VIZ_xmids,
                        st.session_state.VIZ_depths_best_layered,
                        zmin=vmin,
                        zmax=vmax,
                ...
                st.text("")
                st.text("")
                st.error("📁 Inversion data missing for best layered model.")
